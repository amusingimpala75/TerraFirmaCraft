    ...
    ChunkComponentInitializer
{
    ...
    //TODO: Fix always adding to a ReadOnlyChunk -> always EMPTY, thus no data
    @Override
    public void registerChunkComponentFactories(ChunkComponentFactoryRegistry registry) {
        registry.register(CHUNK_DATA, this::getChunkComponent);
    }

    private ChunkDataChunkComponent getChunkComponent(Chunk chunk)
    {
    ...
    public void reinitializeChunk(WorldChunk chunk)
    {
        CHUNK_DATA.maybeGet(chunk).ifPresent(comp -> {
            if (comp.getStatus().ordinal() == 1) {
                System.out.println("Reinitializing Chunk data from empty");
