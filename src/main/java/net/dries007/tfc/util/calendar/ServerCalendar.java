...
public class ServerCalendar extends Calendar
{
     ...
     *
     * @param worldTimeToSetTo a world time, obtained from {@link ServerWorld#getTimeOfDay()}. Must be in [0, ICalendar.TICKS_IN_DAY]
    ...
    void onOverworldTick(ServerWorld world)
    {
        ...
        if (deltaWorldTime > TIME_DESYNC_THRESHOLD || deltaWorldTime < -TIME_DESYNC_THRESHOLD)
        {
            ...
            LOGGER.debug("Calendar Time = {} ({}), Player Time = {}, World Time = {}, doDaylightCycle = {}, ArePlayersLoggedOn = {}", calendarTicks, getCalendarDayTime(), playerTicks, world.getTimeOfDay() % ICalendar.TICKS_IN_DAY, doDaylightCycle, arePlayersLoggedOn);

            // Check if tracking values are wrong
            boolean checkArePlayersLoggedOn = getServer().getPlayerManager().getCurrentPlayerCount() > 0;
