...
{
    public static final EntityType<TFCFallingBlockEntity> FALLING_BLOCK = register("falling_block", EntityType.Builder.<TFCFallingBlockEntity>create(TFCFallingBlockEntity::new, SpawnGroup.MISC).setDimensions(0.98f, 0.98f));
    ...
    public static <E extends Entity> EntityType<E> register(String name, EntityType.Builder<E> builder, boolean serialize)
    {
        ...
        final EntityType<E> type = builder.disableSaving().build(id);
        ((EntityTypeAccessor) type).accessor$setSerialize(serialize);
        return Registry.register(Registry.ENTITY_TYPE, Helpers.identifier(name), type);
    }
