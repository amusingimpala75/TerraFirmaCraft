...
public class CaveSpikesFeature extends Feature<DefaultFeatureConfig>
{
    public CaveSpikesFeature(Codec<DefaultFeatureConfig> codec)
    {
        super(codec);
    }
    ...
    protected void place(StructureWorldAccess worldIn, BlockPos pos, BlockState spike, BlockState raw, Direction direction, Random rand)
    {
        placeSmallSpike(worldIn, pos, spike, raw, direction, rand);
    }

    protected void placeSmallSpike(StructureWorldAccess worldIn, BlockPos pos, BlockState spike, BlockState raw, Direction direction, Random rand)
    {
        placeSmallSpike(worldIn, pos, spike, raw, direction, rand, rand.nextFloat());
    }

    protected void placeSmallSpike(StructureWorldAccess worldIn, BlockPos pos, BlockState spike, BlockState raw, Direction direction, Random rand, float sizeWeight)
    {
    ...
    protected void replaceBlock(StructureWorldAccess world, BlockPos pos, BlockState state)
    {
        ...
        {
            setBlockState(world, pos, state.with(RockSpikeBlock.FLUID, RockSpikeBlock.FLUID.keyFor(Fluids.WATER)));
        }
        else if (block == Blocks.LAVA)
        {
            setBlockState(world, pos, state.with(RockSpikeBlock.FLUID, RockSpikeBlock.FLUID.keyFor(Fluids.LAVA)));
        }
    }

    protected void replaceBlockWithoutFluid(StructureWorldAccess world, BlockPos pos, BlockState state)
    {
        Block block = world.getBlockState(pos).getBlock();
